...
function openSkill(id) {
  ...
  const isWorldCheckin = skill.id === 'world-checkin';
  const imageFields = `<label class="upload-box"><input name="image" type="file" accept="image/png,image/jpeg,image/webp" ${skill.product || isWorldCheckin ? 'required' : ''}><span class="upload-icon">↑</span><b>${t('launcher.upload')}</b><small>${t('launcher.uploadHint')}</small></label>${isWorldCheckin ? `<label>${t('launcher.destination')}<input name="destination" required placeholder="${t('launcher.destinationPlaceholder')}"></label>` : ''}<label>${t('launcher.preset')}<select name="preset"><option>${state.lang === 'vi' ? 'Tự nhiên' : 'Natural'}</option><option>${state.lang === 'vi' ? 'Chuyên nghiệp' : 'Professional'}</option><option>${state.lang === 'vi' ? 'Điện ảnh' : 'Cinematic'}</option><option>${state.lang === 'vi' ? 'Theo yêu cầu' : 'Custom'}</option></select></label><label>${t('launcher.extra')}<textarea name="instruction" placeholder="${isWorldCheckin ? t('launcher.worldCheckinPlaceholder') : t('launcher.extraPlaceholder')}"></textarea></label>`;
...
$('#skill-form').addEventListener('submit', async event => {
  ...
  const realEndpoint = state.activeSkill?.id === 'product-photo' ? '/api/product-photo' : state.activeSkill?.id === 'world-checkin' ? '/api/world-checkin' : null;
  if (!realEndpoint) return setTimeout(() => { box.className = 'result-box success'; box.innerHTML = `<span class="result-icon">✓</span><b>${t('launcher.ready')}</b><p>${t('launcher.readyText')}</p>`; }, 700);
  const formData = new FormData(event.currentTarget); const image = formData.get('image');
  if (!(image instanceof File) || !image.size) { box.className = 'result-box error'; box.innerHTML = `<span class="result-icon">!</span><b>${t(state.activeSkill.id === 'product-photo' ? 'launcher.imageRequired' : 'launcher.referenceImageRequired')}</b>`; return; }
  try {
    const response = await fetch(realEndpoint, { method: 'POST', body: formData }); const payload = await response.json();

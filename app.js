...
function openSkill(id) {
  ...
  const commonImageFields = `<label>${t('launcher.preset')}<select name="preset"><option>${state.lang === 'vi' ? 'Tự nhiên' : 'Natural'}</option><option>${state.lang === 'vi' ? 'Chuyên nghiệp' : 'Professional'}</option><option>${state.lang === 'vi' ? 'Điện ảnh' : 'Cinematic'}</option><option>${state.lang === 'vi' ? 'Theo yêu cầu' : 'Custom'}</option></select></label><label>${t('launcher.extra')}<textarea name="instruction" placeholder="${t('launcher.extraPlaceholder')}"></textarea></label>`;
  const imageFields = skill.id === 'virtual-tryon' ? `${uploadField('personImage', state.lang === 'vi' ? 'Ảnh người' : 'Person image')}${uploadField('garmentImage', state.lang === 'vi' ? 'Ảnh trang phục' : 'Garment image')}${commonImageFields}` : `${uploadField('image', t('launcher.upload'))}${commonImageFields}`;
...
$('#skill-form').addEventListener('submit', async event => {
  ...
  if (state.activeSkill.id !== 'virtual-tryon' && (!(image instanceof File) || !image.size)) { box.className = 'result-box error'; box.innerHTML = `<span class="result-icon">!</span><b>${t(state.activeSkill.id === 'world-checkin' ? 'launcher.referenceRequired' : 'launcher.imageRequired')}</b>`; return; }
  try {

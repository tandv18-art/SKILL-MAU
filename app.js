...
$('#skill-form').addEventListener('submit', async event => {
  ...
  if (REAL_TEXT_SKILLS.has(state.activeSkill?.id)) { box.innerHTML = `<span class="result-icon">◌</span><b>${t('launcher.textWorking')}</b>`; const formData = new FormData(event.currentTarget); const input = [formData.get('input'), formData.get('context')].filter(Boolean).join('\n\nContext: '); try { const response = await fetch('/api/text-skill', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({skillId:state.activeSkill.id,input,language:state.lang}) }); const payload = await response.json(); if (!response.ok || !payload.success || typeof payload.output !== 'string') throw new Error('Text generation failed'); const pre = document.createElement('pre'); pre.className = 'text-result'; pre.textContent = payload.output; box.className = 'result-box success text-results'; box.replaceChildren(pre); $('[data-regenerate]').hidden = false; } catch { box.className = 'result-box error'; box.innerHTML = `<span class="result-icon">!</span><b>${t('launcher.textGenerationError')}</b><p>${t('launcher.textTryAgain')}</p>`; $('[data-regenerate]').hidden = false; } return; }

...
function openSkill(id) {
  ...
  const uploadField = (name, label) => `<div class="upload-control"><label class="upload-box" id="upload-drop-${name}"><input data-image-input="${name}" name="${name}" type="file" accept="image/png,image/jpeg,image/webp" required><span class="upload-icon">↑</span><b>${label}</b><small>${t('launcher.uploadHint')}</small></label><div class="upload-preview" id="upload-preview-${name}" hidden><img id="upload-thumbnail-${name}" alt="Ảnh đã chọn"><div><b id="upload-filename-${name}"></b><small id="upload-filesize-${name}"></small><span><button type="button" data-image-action="replace" data-image-key="${name}">Thay ảnh</button><button type="button" data-image-action="remove" data-image-key="${name}">Xóa</button></span></div></div><p class="field-error" id="upload-error-${name}" role="alert"></p></div>`;
  const instructionField = `<label>${t('launcher.extra')}<textarea name="instruction" placeholder="${t('launcher.extraPlaceholder')}"></textarea></label>`;
  const presetField = `<label>${t('launcher.preset')}<select name="preset"><option>${state.lang === 'vi' ? 'Tự nhiên' : 'Natural'}</option><option>${state.lang === 'vi' ? 'Chuyên nghiệp' : 'Professional'}</option><option>${state.lang === 'vi' ? 'Điện ảnh' : 'Cinematic'}</option><option>${state.lang === 'vi' ? 'Theo yêu cầu' : 'Custom'}</option></select></label>`;
  const imageFields = skill.id === 'virtual-tryon' ? `${uploadField('personImage', state.lang === 'vi' ? 'Ảnh người' : 'Person image')}${uploadField('garmentImage', state.lang === 'vi' ? 'Ảnh trang phục' : 'Garment image')}${instructionField}` : `${uploadField('image', t('launcher.upload'))}${presetField}${instructionField}`;
...
function clearSelectedImage(key) { const keys = key ? [key] : Object.keys(state.selectedImageUrls); keys.forEach(name => { if (state.selectedImageUrls[name]) URL.revokeObjectURL(state.selectedImageUrls[name]); delete state.selectedImageUrls[name]; const input = $(`[data-image-input="${name}"]`); if (input) input.value = ''; const preview = $(`#upload-preview-${name}`); if (preview) preview.hidden = true; const drop = $(`#upload-drop-${name}`); if (drop) drop.hidden = false; const error = $(`#upload-error-${name}`); if (error) error.textContent = ''; }); }
function selectImage(file, key) { const allowed = ['image/jpeg','image/png','image/webp']; if (!file || !allowed.includes(file.type)) { clearSelectedImage(key); $(`#upload-error-${key}`).textContent = state.lang === 'vi' ? 'Chỉ hỗ trợ JPG, PNG hoặc WEBP.' : 'Use a JPG, PNG, or WEBP image.'; return; } if (state.selectedImageUrls[key]) URL.revokeObjectURL(state.selectedImageUrls[key]); state.selectedImageUrls[key] = URL.createObjectURL(file); $(`#upload-thumbnail-${key}`).src = state.selectedImageUrls[key]; $(`#upload-filename-${key}`).textContent = file.name; $(`#upload-filesize-${key}`).textContent = `${(file.size / 1024 / 1024).toFixed(2)} MB`; $(`#upload-preview-${key}`).hidden = false; $(`#upload-drop-${key}`).hidden = true; }
...
document.addEventListener('change', event => { if (event.target.matches('[data-image-input]')) selectImage(event.target.files[0], event.target.dataset.imageInput); });
...
function renderTextRuntimeMessage(box, message, needsFacts = false) { const icon = document.createElement('span'); icon.className = 'result-icon'; icon.textContent = needsFacts ? '?' : '!'; const text = document.createElement('b'); text.textContent = message; box.className = `result-box ${needsFacts ? 'guidance' : 'error'}`; box.replaceChildren(icon, text); }
$('#skill-form').addEventListener('submit', async event => {
  ...
  if (REAL_TEXT_SKILLS.has(state.activeSkill?.id)) { box.innerHTML = `<span class="result-icon">◌</span><b>${t('launcher.textWorking')}</b>`; const formData = new FormData(event.currentTarget); const input = [formData.get('input'), formData.get('context')].filter(Boolean).join('\n\nContext: '); try { const response = await fetch('/api/text-skill', { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({skillId:state.activeSkill.id,input,language:state.lang}) }); const payload = await response.json(); if (!response.ok || !payload.success) { renderTextRuntimeMessage(box, typeof payload.error === 'string' ? payload.error : t('launcher.textGenerationError'), payload.code === 'NEED_MORE_FACTS'); $('[data-regenerate]').hidden = false; return; } if (typeof payload.output !== 'string') throw new Error('Text generation failed'); const pre = document.createElement('pre'); pre.className = 'text-result'; pre.textContent = payload.output; box.className = 'result-box success text-results'; box.replaceChildren(pre); $('[data-regenerate]').hidden = false; } catch { renderTextRuntimeMessage(box, t('launcher.textGenerationError')); $('[data-regenerate]').hidden = false; } return; }
  if (!IMAGE_ENDPOINTS[state.activeSkill?.id]) return setTimeout(() => { box.className = 'result-box success'; box.innerHTML = `<span class="result-icon">✓</span><b>${t('launcher.ready')}</b><p>${t('launcher.readyText')}</p>`; $('[data-regenerate]').hidden = false; }, 700);
  const formData = new FormData(event.currentTarget); const image = formData.get('image');
  if (state.activeSkill.id !== 'virtual-tryon' && (!(image instanceof File) || !image.size)) { const requiredKey = state.activeSkill.id === 'world-checkin' ? 'launcher.referenceRequired' : state.activeSkill.id === 'premium-portrait-enhancer' ? 'launcher.portraitRequired' : 'launcher.imageRequired'; box.className = 'result-box error'; box.innerHTML = `<span class="result-icon">!</span><b>${t(requiredKey)}</b>`; return; }
  try {

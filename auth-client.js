(() => {
  ...
  function openAuthReal(mode = 'login') {
    ...
    if (mode === 'signup') {
      ...
      copy.textContent = text('Tạo tài khoản TÔI LÀ AI bằng email và mật khẩu.', 'Create your TÔI LÀ AI account with email and password.');
      ...
      copy.textContent = text('Tiếp tục vào không gian làm việc TÔI LÀ AI.', 'Continue to your TÔI LÀ AI workspace.');
  ...
  function renderWorkspace(section = 'home') {
    ...
    if (section === 'account') {
      workspaceCard([user.name || text('Tài khoản TÔI LÀ AI', 'TÔI LÀ AI account'), user.email || '', text('Phiên đăng nhập đang hoạt động và được bảo vệ bằng cookie HttpOnly.', 'Your session is active and protected by an HttpOnly cookie.')]);
      const box = $('#workspace-content');

...
async function requireSiteOwner(req) {
  ...
  if (hasAdminRole(session?.user?.role)) {
    return { ok: true, session, userId, authorization: 'admin-role' };
  }
  ...
  try {
    ...
    return { ok: true, session, userId, authorization: 'legacy-owner' };
  } catch (error) {
...
module.exports = { requireSiteOwner, hasAdminRole };

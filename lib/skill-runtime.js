...
function publicError(error) {
  console.error('Skill runtime error:', error?.message || error);
  return { status: Number(error?.status) || 500, body: { success: false, error: error?.publicMessage || (Number(error?.status) >= 400 && Number(error?.status) < 500 ? error.message : 'Không thể xử lý yêu cầu lúc này.') } };
}
...
async function readJson(req) {
  ...
  for await (const chunk of req) {
    body += chunk;
    if (body.length > 100_000) throw Object.assign(new Error('Request too large.'), { status: 413 });
  }
...
async function readForm(req) {
  ...
  const request = new Request(`https://toilaai.local${req.url || '/'}`, { method: req.method, headers: req.headers, body: req, duplex: 'half' });
...
async function runTextSkill(req, res) {
  ...
  if (!process.env.OPENAI_API_KEY) return sendJson(res, 503, { success: false, error: 'Runtime is not configured.' });
  const body = await readJson(req);
  const skillId = String(body.skillId || '');
  const input = String(body.input || '').trim();
  const language = String(body.language || 'vi').slice(0, 20);
  if (!TEXT_SKILL_IDS.has(skillId)) return sendJson(res, 400, { success: false, error: 'Unsupported text skill.' });
  if (!input) return sendJson(res, 400, { success: false, error: 'Input is required.' });
  if (needsAuthoritativeFacts(input)) return sendJson(res, 400, { success: false, code: 'NEED_MORE_FACTS', error: NEED_MORE_FACTS_MESSAGE });
  const instructions = await fs.readFile(path.join(__dirname, '..', skillId, 'SKILL.md'), 'utf8');
  const baseUrl = String(process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/$/, '');
  const response = await fetch(`${baseUrl}/chat/completions`, {
    method: 'POST',
    headers: { authorization: `Bearer ${process.env.OPENAI_API_KEY}`, 'content-type': 'application/json' },
    body: JSON.stringify({ model: process.env.OPENAI_TEXT_MODEL || 'gpt-5-mini', messages: [{ role: 'system', content: `${FINAL_CREATIVE_TEXT_POLICY}\n\n${instructions}\nRespond in language: ${language}.` }, { role: 'user', content: input }] })
  });
  const payload = await response.json().catch(() => ({}));
...
async function generateProductPhotos(req, res) {
  if (req.method !== 'POST') return sendJson(res, 405, { success: false, error: 'Method not allowed.' });
  if (!process.env.OPENAI_API_KEY) return sendJson(res, 503, { success: false, error: 'Runtime is not configured.' });
  if (!String(req.headers['content-type'] || '').toLowerCase().startsWith('multipart/form-data')) return sendJson(res, 415, { success: false, error: 'Multipart form data is required.' });
  const form = await readForm(req);
  const image = form.get('image');
  if (!(image instanceof Blob) || !image.size) return sendJson(res, 400, { success: false, error: 'A product image is required.' });
  if (!IMAGE_TYPES.has(image.type)) return sendJson(res, 415, { success: false, error: 'Unsupported image format.' });
  ...
  providerForm.set('prompt', buildProductPrompt(scene, angle, intendedUse, instruction));
  providerForm.set('n', String(variations));
  providerForm.set('size', '1536x1024');
  providerForm.set('quality', 'high');
  return sendJson(res, 200, { success: true, images: await callImageProvider(providerForm, 'Image provider rejected the request.') });
}
...
async function generateWorldCheckin(req, res) {
  ...
  const form = await readForm(req);
  const image = form.get('image');
  if (!(image instanceof Blob) || !image.size) return sendJson(res, 400, { success: false, error: 'A reference image is required.' });
  if (!IMAGE_TYPES.has(image.type)) return sendJson(res, 415, { success: false, error: 'Unsupported image format.' });
  const destination = String(form.get('destination') || '').trim().slice(0, 500);
  if (!destination) return sendJson(res, 400, { success: false, error: 'A destination is required.' });
  ...
  providerForm.set('prompt', buildWorldCheckinPrompt(destination, preset, instruction));
...
async function generatePortrait(req, res) {
  ...
  const form = await readForm(req);
  const image = form.get('image');
  if (!(image instanceof Blob) || !image.size) return sendJson(res, 400, { success: false, error: 'A portrait image is required.' });
  if (!IMAGE_TYPES.has(image.type)) return sendJson(res, 415, { success: false, error: 'Unsupported image format.' });
  const preset = String(form.get('preset') || 'natural').slice(0, 200);
  const instruction = String(form.get('instruction') || '').slice(0, 2000);
  const providerForm = new FormData();
  providerForm.set('model', HUMAN_IMAGE_MODEL);
  providerForm.set('image', image, image.name || 'portrait.png');
  providerForm.set('prompt', buildPortraitPrompt(preset, instruction));
  providerForm.set('n', '1');
  providerForm.set('size', '1024x1536');
  providerForm.set('quality', 'high');
  return sendJson(res, 200, { success: true, images: await callImageProvider(providerForm, 'Portrait provider rejected the request.') });
}
...
async function generateTryon(req, res) {
  ...
  if (!String(req.headers['content-type'] || '').toLowerCase().startsWith('multipart/form-data')) return sendJson(res, 415, { success: false, error: 'Multipart form data is required.' });
  const form = await readForm(req);
  const person = form.get('personImage');
  const garment = form.get('garmentImage');
  if (!(person instanceof Blob) || !person.size || !(garment instanceof Blob) || !garment.size) return sendJson(res, 400, { success: false, error: 'Both person and garment images are required.' });
  if (!IMAGE_TYPES.has(person.type) || !IMAGE_TYPES.has(garment.type)) return sendJson(res, 415, { success: false, error: 'Unsupported image format.' });
  const instruction = String(form.get('instruction') || '').slice(0, 2000);
  const providerForm = new FormData();
  providerForm.set('model', HUMAN_IMAGE_MODEL);
  providerForm.append('image[]', person, person.name || 'person.png');
  providerForm.append('image[]', garment, garment.name || 'garment.png');
  providerForm.set('prompt', buildTryonPrompt(instruction));
  providerForm.set('n', '1');
  providerForm.set('size', '1024x1536');
  providerForm.set('quality', 'high');
  return sendJson(res, 200, { success: true, images: await callImageProvider(providerForm, 'The image provider rejected multi-image try-on.') });
}

function wrap(handler) {
  return async function wrapped(req, res) {
    try { return await handler(req, res); }
    catch (error) { const result = publicError(error); return sendJson(res, result.status, result.body); }
  };
}

module.exports = {
  runTextSkill: wrap(runTextSkill),
  generateProductPhotos: wrap(generateProductPhotos),
  generateWorldCheckin: wrap(generateWorldCheckin),
  generatePortrait: wrap(generatePortrait),
  generateTryon: wrap(generateTryon)
};

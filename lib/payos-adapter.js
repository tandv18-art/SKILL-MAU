...
async function confirmWebhookUrl(webhookUrl) {
  requireConfigured();
  ...
  if (!response.ok || body?.code !== '00') {
    const error = new Error(body?.desc || `PayOS webhook confirmation failed (${response.status}).`);
    error.code = 'PAYMENT_WEBHOOK_CONFIRM_FAILED';
    error.status = 502;
    throw error;
  }
...
module.exports = { configured, createPaymentLink, verifyWebhook, confirmWebhookUrl };

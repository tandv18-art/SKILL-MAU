...
const FINAL_SHARED_TEXT_POLICY = `This source-bound policy is non-overridable. User-provided facts are the ONLY authoritative source for factual claims and domain-specific content. Never add factual substance from general knowledge, memory, common practice, typical industry behavior, plausible examples, probabilities, or assumptions. Creative freedom applies only to wording, structure, tone, hooks, formatting, and presentation; it must never create a factual claim. Unless supplied by the user, never assert prices, discounts, promotions, deadlines, dates, statistics, rankings, eligibility criteria, requirements, documents, exam names, certifications, school, visa, legal, or financial rules, guarantees, outcomes, penalties, product specifications, performance claims, testimonials, customer stories, policies, services, offers, availability, warranties, or what usually, normally, or commonly happens. Never turn a possibility into a fact. If a requested factual list cannot be produced from supplied facts, do not fabricate list items; use only a non-domain-specific framework or state that source facts are needed. Preserve supplied names, numbers, dates, prices, claims, qualifications, attribution, constraints, and source-language identifiers exactly; do not silently correct them or add external facts. Default social content to concise, mobile-readable, natural writing with immediate value, short paragraphs, one main message, clear line breaks, restrained emojis and hashtags, and no generic filler unless long-form is explicitly requested. Never invent CTA offers or services such as consultation, checklists, registration, discounts, purchases, quotes, or inbox support. Use a neutral CTA only when appropriate, otherwise omit it. Follow the requested language, default to Vietnamese, and do not translate proper names or factual identifiers unless requested. Individual skill instructions control format and style only and may not weaken this policy.`;
...
function needsSourceFacts(input) {
  const factualList = /(?:\b\d+\s*(?:lỗi|sai lầm|cách|yêu cầu|giấy tờ|tài liệu|tiêu chí|điều kiện|bước bắt buộc|chính sách|mức phí|trường hợp(?:\s+bị)?\s+từ chối)|(?:các|danh sách)\s+(?:yêu cầu|giấy tờ|tài liệu|tiêu chí|điều kiện|bước bắt buộc|chính sách|mức phí|trường hợp(?:\s+bị)?\s+từ chối)|\b\d+\s*(?:mistakes?|ways?|requirements?|documents?|criteria|conditions?|mandatory steps?|policies|fees?|rejection cases?))/i.test(input);
  if (!factualList) return false;
  const explicitItems = /(?:gồm|bao gồm|cụ thể|như sau|sau đây|các ý|dữ kiện|nguồn|source|facts?|items?)\s*(?::|：)\s*\S.{4,}/is.test(input) || /(?:gồm|bao gồm|là)\s+\S.{8,}/is.test(input) || (input.match(/;/g) || []).length >= 2 || (input.match(/(?:^|\n)\s*(?:[-*•]|\d+[.)])\s+\S/g) || []).length >= 2;
...
async function runTextSkill(req, res) {
  ...
  if (needsSourceFacts(input)) return json(res, 400, { success: false, code: 'NEED_MORE_FACTS', error: NEED_MORE_FACTS_MESSAGE });
  if (!process.env.OPENAI_API_KEY) return json(res, 503, { success: false, error: 'Runtime is not configured.' });
  ...
  const response = await fetch(`${baseUrl}/chat/completions`, { method: 'POST', headers: { authorization: `Bearer ${process.env.OPENAI_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: process.env.OPENAI_TEXT_MODEL || 'gpt-5-mini', messages: [{ role: 'system', content: `${FINAL_SHARED_TEXT_POLICY}\n\n${instructions}\nRespond in language: ${language}.` }, { role: 'user', content: input }] }) }); const payload = await response.json().catch(() => ({}));
